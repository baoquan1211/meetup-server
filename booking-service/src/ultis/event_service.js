class EventService {
  static FindSeat = async (seat_id) => {
    const url = `http://event-service:7000/api/v1/seat/${id}`;
  ...
  static FindSeats = async (event_id, status) => {
    const url = `http://event-service:7000/api/v1/seats/${event_id}?status=${status}`;
    const response = await fetch(url);
    if (response.status != 200) {
      return null;
    }
  ...
  static FindEvent = async (id) => {
    const response = await fetch(
      `http://event-service:7000/api/v1/event/${id}`
    );
    if (response.status != 200) {
      return null;
    }
