...
EventRouter = APIRouter(tags=["View - Event"], dependencies=[Depends(get_db)])
...
def list_events(Form: EventQuery = Depends(), db=Depends(get_db)):
    ...
    for event in events:
        event.pop("about")
        event["attends"] = len(
            SeatService(db).all(Event_ID=event["id"], Status="Ordered")
        )
        event["left"] = len(
            SeatService(db).all(Event_ID=event["id"], Status="NotOrdered")
        )
        event["agenda"] = ShowService(db).all(Event_ID=event["id"])
    return Response(metadata=events)
...
def view_event(Event_ID: int, db=Depends(get_db)):
    ...
    event["agenda"] = ShowService(db).all(Event_ID=event["id"])
    return Response(metadata=event)
