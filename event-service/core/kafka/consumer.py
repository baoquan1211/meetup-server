...
class KafkaConsumer:
    ...
    async def __update_profile__(self, msg):
        ...
        first_name = (
            value["first_name"]
            if value["first_name"]
            else event.split(" ")[0] if event is not None else ""
        )
        last_name = (
            value["last_name"]
            if value["last_name"]
            else event.split(" ")[1] if event is not None else ""
        )
    ...
    async def __booking__(self, msg):
        ...
        async with async_get_db() as db:
            ...
            if "voucher_id" in booking_detail:
                voucher = (
                    db.query(Voucher)
                    .filter(Voucher.id == booking_detail["voucher_id"])
                    .first()
                )
                voucher.remaining -= 1
            db.commit()
    ...
    async def __payment__(self, msg):
        ...
        if status == "Failed":
            ...
            async with async_get_db() as db:
                seat = (
                    db.query(Seat).filter(Seat.id == booking_detail["seat_id"]).first()
                )
                if "voucher_id" in booking_detail:
                    voucher = (
                        db.query(Voucher)
