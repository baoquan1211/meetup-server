...
function Header() {
  ...
  return (
    <header className="sticky left-0 top-0 z-50 flex h-16 w-full items-center justify-between border-b-[1px] border-muted bg-background px-2 md:px-6">
      ...
      authSignal.value.accessToken.value?.length > 0 ? (
        <div className="flex gap-1 md:gap-6">
          {/* <Button
            onClick={() =>
              window.open(
                "https://resume31337.s3.ap-southeast-1.amazonaws.com/RoboTasker.zip"
              )
            }
            variant={"outline"}
            className="hidden items-center gap-2 border-primary text-primary hover:text-primary/80 md:flex"
          >
            <Download className="size-4" /> {translate("Download App")}
          </Button> */}
        ...
        <div className="hidden items-center gap-6 md:flex">
          {location.pathname !== "/login" && (
            <Link
              to={"/login"}
              className="transition-200 font-semibold hover:text-secondary"
            >
              {"Login"}
            </Link>
          )}
          {location.pathname !== "/sign-up" && (
            <Link to="/sign-up">
              <Button className="px-6 text-lg font-semibold bg-secondary hover:bg-secondary/90">
                {"Sign up"}
              </Button>
            </Link>
          )}
